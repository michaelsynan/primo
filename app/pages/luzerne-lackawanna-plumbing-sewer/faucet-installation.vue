<script setup lang="ts"></script>

<template>
  <div class="flex flex-col justify-center gap-4 min-h-screen max-w-4xl mx-auto px-4">
    <h1 class="text-4xl fancy">Faucet Installation</h1>
    <NuxtImg src="/faucet-installation-plumber.webp" alt="High Pressure Water Jetting"
      class="h-80 object-cover object-top rounded" />

    <div class="roboto text-lg leading-loose">Our technicians fully inspect and properly diagnose your issue at low cost
      with
      professionalism & enthusiasm for all your clogs, leaks, and repairs!</div>
    <div>
      <ul class="flex flex-col gap-2">
        <li>Item 1</li>
        <li>Item 2</li>
        <li>Item 3</li>
        <li>Item 4</li>
      </ul>
    </div>
    <div class="mt-4 md:w-max">
      <SharedButton text="Contact Us" />
    </div>
  </div>
</template>

<style scoped></style>
